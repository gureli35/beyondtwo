...
<p>Yazınızı güçlü bir sonuçla bitirin ve okuyucularınızı eyleme geçmeye teşvik edin.</p>`,
  ...
  const handleSave = async (formData: any) => {
    try {
      ...
      const tagsArray = formData.tags ? formData.tags.split(',').map((tag: string) => tag.trim()).filter((tag: string) => tag.length > 0) : [];
      ...
      const payload = {
        ...
        metaKeywords: Array.isArray(formData.metaKeywords) ? formData.metaKeywords : [],
        readingTime: formData.readingTime || 0,
        featured: Boolean(formData.featured)
      };

      console.log('Creating new blog post with payload:', payload);
      ...
      if (result.success) {
        ...
        throw new Error(result.error || 'Blog yazısı oluşturulamadı');
      }
    } catch (error: any) {
